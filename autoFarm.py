...
def main():
    ...
    if result:
        ...
        print("没找到blum服务确认按钮")
    time.sleep(15)
    result =foundBlumProgram.locate_image_on_screen(blumBlum_path)
    if result:
        clickPosition.click_at_position(result)
        print(f"找到blum服务确认按钮")
    else:
    ...
    while True:
        ...
        time.sleep(1)
        result =foundBlumProgram.locate_image_on_screen(blumPlay_path)
        if result:
            print(f"有票，开始玩游戏")
            clickPosition.click_at_position(result)
            time.sleep(1)
            blumGameBoot.main()
            result = foundBlumProgram.locate_image_on_screen(blumBack_path)
            clickPosition.click_at_position(result)
            time.sleep(1)
        else:
            print("没票了")
